...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="bg-card border-b border-border shadow-sm">
        <div className="container mx-auto px-4 flex items-center gap-4 md:gap-6 h-16 md:h-[72px]">
          ...
          <div className="hidden md:flex flex-1 max-w-xl relative">
            ...
            />
            <button className="absolute right-1.5 top-1.5 bottom-1.5 px-3 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-smooth">
          ...
          <div className="flex items-center gap-1 md:gap-4 ml-auto">
            ...
            <a href="#" className="hidden md:flex items-center justify-center w-10 h-10 rounded-lg text-muted-foreground hover:bg-nipo-pink-light hover:text-secondary transition-smooth">
      ...
      {/* Navigation bar */}
      <div className="bg-card border-b border-border/60 hidden md:block">
        <div className="container mx-auto px-4 flex items-center h-11 gap-0">
              ...
              {catOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute top-full left-0 w-72 bg-card border border-border rounded-b-xl shadow-nipo-lg z-50"
                  >
                    {categories.map((cat) => (
                      <Link
                        key={cat}
          ...
          {/* Nav items */}
          <nav className="flex items-center h-full ml-4 gap-0">
            {navItems.map((item) => (
              <Link
                key={item.label}
                to={item.href}
                className="relative flex items-center gap-1 px-4 h-full text-[13px] font-medium text-foreground/80 hover:text-primary transition-smooth group"
              >
                {item.label}
                {item.hasSub && <ChevronDown className="w-3 h-3" />}
                <span className="absolute bottom-0 left-4 right-4 h-0.5 bg-secondary scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left rounded-full" />
              </Link>
            ))}
          </nav>
