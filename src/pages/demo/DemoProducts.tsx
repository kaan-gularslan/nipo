      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-14">
        ...
        <div className="lg:hidden flex gap-2 overflow-x-auto pb-2 scrollbar-thin -mx-4 px-4">
          <button
            ...
            className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-smooth ${!selectedCat ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
          >
          ...
          {categories.map((cat) => {
            ...
            return (
              <button key={cat.id} onClick={() => setSelectedCat(cat.id)} className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-smooth ${selectedCat === cat.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
        ...
        <aside className="hidden lg:block lg:w-64 shrink-0">
          <div className="bg-card rounded-xl border border-border/60 overflow-hidden sticky top-36">
            ...
            <ul>
              <li>
                <button onClick={() => setSelectedCat(null)} className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-xs border-b border-border/20 transition-smooth text-left ${!selectedCat ? "bg-primary/10 text-primary font-bold" : "text-foreground hover:bg-muted hover:text-primary"}`}>
                  <span>📋</span><span className="flex-1">Tüm Ürünler</span><span className="text-[10px] text-muted-foreground">{products.length}</span>
                </button>
              </li>
              {categories.map((cat) => {
                const count = products.filter(p => p.categoryId === cat.id).length;
                ...
                return (
                  <li key={cat.id}>
                    <button onClick={() => setSelectedCat(cat.id)} className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-xs border-b border-border/20 transition-smooth text-left ${selectedCat === cat.id ? "bg-primary/10 text-primary font-bold" : "text-foreground hover:bg-muted hover:text-primary"}`}>
                      <span>{cat.icon}</span><span className="flex-1">{cat.name}</span><span className="text-[10px] text-muted-foreground">{count}</span>
                    </button>
                  </li>
                );
              })}
              ...
              <article key={product.id} className="group bg-card rounded-xl border border-border/60 overflow-hidden card-hover">
                <Link to={demoLink(`/urunler/${product.slug}`)}>
                  <div className="relative aspect-square overflow-hidden bg-muted/30">
                    ...
                    {product.badge && (
                      <span className={`absolute top-2.5 left-2.5 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide shadow-sm ${product.badge === "Çok Satan" ? "bg-secondary text-secondary-foreground" : product.badge === "Yeni" ? "bg-accent text-accent-foreground" : "bg-primary text-primary-foreground"}`}>{product.badge}</span>
                    )}
                    {product.oldPrice && (
                      <span className="absolute top-2.5 right-2.5 w-10 h-10 rounded-full bg-secondary text-secondary-foreground text-[10px] font-bold flex items-center justify-center shadow-sm">%{getDiscountPercent(product.price, product.oldPrice)}</span>
                    )}
                  </div>
                </Link>
                <div className="p-4">
